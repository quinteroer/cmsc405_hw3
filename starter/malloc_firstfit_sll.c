...
static block_meta_t *data_to_block(void *ptr) {
...
static void *thread_worker(void *arg) {
    ...
    for (int i = 0; i < 10; ++i) {
        a[i] = ff_malloc((size_t)(32 + t->id * 8 + i * 4));
        c[i] = ff_calloc((size_t)(i + 1), sizeof(int));
    }
    for (int i = 0; i < 10; ++i) {
        a[i] = ff_realloc(a[i], (size_t)(64 + i * 8));
    }
    for (int i = 0; i < 10; ++i) {
        ff_free(a[i]);
        ff_free(c[i]);
...
int main(void) {
    ...
    void *c[10] = {0};
    for (int i = 0; i < 10; ++i) {
        m[i] = ff_malloc((size_t)(24 + i * 8));
    }
    for (int i = 0; i < 10; ++i) {
        c[i] = ff_calloc((size_t)(3 + i), sizeof(int));
    }
    for (int i = 0; i < 10; ++i) {
        m[i] = ff_realloc(m[i], (size_t)(96 + i * 8));
    }
    for (int i = 0; i < 10; ++i) {
        ff_free(m[i]);
        ff_free(c[i]);
    ...
    printf("[first-fit/sll] heap start: %p\n", heap_start);
    printf("[first-fit/sll] heap end  : %p\n", sbrk(0));
    printf("[first-fit/sll] leaked bytes: %zu\n", leaked_bytes());
    return 0;
}
