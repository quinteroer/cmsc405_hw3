...
static int are_adjacent(block_meta_t *a, block_meta_t *b) { return (char *)(a + 1) + a->size == (char *)b; }

static block_meta_t *request_space(block_meta_t *last, size_t size) {
    void *brk_before = sbrk(0);
    if (sbrk((intptr_t)(sizeof(block_meta_t) + size)) == (void *)-1) return NULL;
    ...
    if (!heap_start) heap_start = brk_before;
    return block;
}
...
static size_t leaked_bytes(void) {
    size_t leaked = 0;
    pthread_mutex_lock(&heap_lock);
    for (block_meta_t *b = global_base; b; b = b->next) if (!b->free) leaked += b->size;
    pthread_mutex_unlock(&heap_lock);
    return leaked;
}
...
static void *thread_worker(void *arg) {
    ...
    void *c[10] = {0};
    for (int i = 0; i < 10; ++i) {
        m[i] = wf_malloc((size_t)(28 + t->id * 6 + i * 5));
        c[i] = wf_calloc((size_t)(i + 3), sizeof(int));
    }
    for (int i = 0; i < 10; ++i) m[i] = wf_realloc(m[i], (size_t)(88 + i * 8));
    for (int i = 0; i < 10; ++i) {
        wf_free(m[i]);
        wf_free(c[i]);
    }
...
int main(void) {
    void *m[10] = {0};
    void *c[10] = {0};
    for (int i = 0; i < 10; ++i) m[i] = wf_malloc((size_t)(30 + i * 7));
    for (int i = 0; i < 10; ++i) c[i] = wf_calloc((size_t)(2 + i), sizeof(int));
    for (int i = 0; i < 10; ++i) m[i] = wf_realloc(m[i], (size_t)(70 + i * 12));
    for (int i = 0; i < 10; ++i) {
        wf_free(m[i]);
        wf_free(c[i]);
    }
    ...
    printf("[worst-fit/dll] heap start: %p\n", heap_start);
    printf("[worst-fit/dll] heap end  : %p\n", sbrk(0));
    printf("[worst-fit/dll] leaked bytes: %zu\n", leaked_bytes());
    return 0;
}
